"""Nucleus MCP server (stdio transport: zero open ports, zero listeners).
...
Credential resolution at startup: macOS Keychain → NUCLEUS_PASSPHRASE env.
    ...
    if v is None or v._locked:
        ...
        try:
            ...
            raise VaultLockedError(
                "Vault is locked. Run `nucleus unlock` on the machine, "
    ...
    try:
        ...
        if not cfg.settings.get("unlock_tool_enabled", False):
            return json.dumps({"error": "Disabled",
                               ...
                               "unlock out-of-band with `nucleus unlock` instead "
...
def main(argv: list[str] | None = None) -> None:
    ap = argparse.ArgumentParser(prog="nucleus serve")
