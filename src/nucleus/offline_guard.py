...
When activated (NUCLEUS_ASSERT_OFFLINE=1 or --assert-offline), creating any
...
def _guarded_new(cls, family=-1, type=-1, proto=-1, fileno=None):  # noqa: A002
    ...
    if fam in (socket.AF_INET, socket.AF_INET6):
        raise OfflineViolation(
            ...
            "Nucleus never touches the network at runtime; aborting."
        )
...
def activate_from_env() -> None:
    if os.environ.get("NUCLEUS_ASSERT_OFFLINE") == "1":
        activate()
