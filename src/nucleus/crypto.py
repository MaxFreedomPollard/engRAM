"""Nucleus cryptography: XChaCha20-Poly1305 AEAD, Argon2id KDF, LUKS-style keyslots.
...
def make_passphrase_slot(master_key: bytes, passphrase: str, kdf: dict | None = None) -> dict:
    ...
    return {
        ...
        "kdf": kdf,
        "salt": salt.hex(),
        "wrapped": seal(wrap_key, master_key, aad=b"nucleus-keyslot").hex(),
    }
...
def open_slot(slot: dict, secret: str) -> bytes:
    ...
    return unseal(wrap_key, bytes.fromhex(slot["wrapped"]), aad=b"nucleus-keyslot")
...
def new_record_key(master_key: bytes, record_id: str) -> tuple[bytes, bytes]:
    ...
    rk = new_key()
    wrapped = seal(master_key, rk, aad=b"nucleus-record:" + record_id.encode())
    return rk, wrapped


def unwrap_record_key(master_key: bytes, record_id: str, wrapped: bytes) -> bytes:
    return unseal(master_key, wrapped, aad=b"nucleus-record:" + record_id.encode())
