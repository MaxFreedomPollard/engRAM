...
class VaultFormatError(CryptoError):
    """The file is not a valid Nucleus vault (or a newer, unknown version)."""
...
def _payload_aad(vault_id: str) -> bytes:
    return b"nucleus-payload:" + vault_id.encode()


def _journal_aad(vault_id: str, seq: int) -> bytes:
    return b"nucleus-journal:" + vault_id.encode() + b":" + struct.pack(">Q", seq)
...
def read_vault_file(path: str) -> LoadedVaultFile:
    ...
    if len(raw) < 10 or raw[:4] != MAGIC:
        raise VaultFormatError(f"{path} is not a Nucleus vault (bad magic)")
